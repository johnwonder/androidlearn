...
public class XCArcProgressBar extends View{
	...
	public XCArcProgressBar(Context context, AttributeSet attrs,
		...
		textColor  =typedArray.getColor(R.styleable.XCRoundProgressBar_textColor, Color.GREEN);
	...
	protected void onDraw(Canvas canvas) {
		...
		degrees = progress * 270 / max - 270;//旋转角度
		//遮罩处理前景图和背景图
		can.save();
		can.rotate(degrees, centerX, centerY);
		//paint.setAntiAlias(true);
		paint.setXfermode(new PorterDuffXfermode(Mode.SRC_ATOP));//取下层非交集部分与上层交集部分
...
//				Bitmap src = bmpBg.createBitmap(bmpBg, 0, 0, 150, centerX);
